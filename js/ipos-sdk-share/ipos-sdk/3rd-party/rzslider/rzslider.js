...
angular.module('rzModule', ['filterUIModule'])
...
.factory('RzSlider', ['$timeout', '$document', '$window', 'throttle', '$filter', function($timeout, $document, $window, throttle, $filter)
{
  'use strict';
  ...
  var Slider = function(scope, sliderElem, attributes)
  {
    ...
    if(this.scope.sliderCeilPercentIncrease)
      this.valueChange = this.scope.rzSliderCeil*this.scope.sliderCeilPercentIncrease/100;
    else 
      this.valueChange = this.scope.rzSliderCeil*20/100;
...
//      this.scope.$apply();
    ...
    translateFn: function(value, label, useCustomTr)
    {
      ...
      if(this.scope.rzSliderFormat == "currency"){
    	  label.text($filter(this.scope.rzSliderFormat)(valStr, '', 0));
      }else{
    	  label.text(valStr);
      }
    ...
    setMinAndMax: function()
    {
      ...
      {
        /*if(this.scope.rzSliderModel > this.scope.rzSliderCeil)
          this.scope.rzSliderCeil = this.maxValue = this.scope.rzSliderModel*2;
        else this.maxValue = +this.scope.rzSliderCeil;*/
         this.maxValue = +this.scope.rzSliderCeil;
      }
      else
      {
        this.scope.rzSliderCeil = this.maxValue = this.range ? this.scope.rzSliderHigh*2 : this.scope.rzSliderModel*2;
      }
    ...
    onEnd: function(ehMove, event)
    {
      ...
      this.tracking = '';
      if(this.scope.rzSliderUnlimit){
        if((this.scope.rzSliderModel/this.scope.rzSliderCeil)*100 > 80){
          if(this.scope.sliderCeilPercentIncrease)
            this.scope.rzSliderCeil += this.valueChange;
        }
              
        if((this.scope.rzSliderModel/this.scope.rzSliderCeil)*100 < 20){
          if(this.scope.sliderCeilPercentIncrease)
            this.scope.rzSliderCeil -= this.valueChange;
        }
      }
